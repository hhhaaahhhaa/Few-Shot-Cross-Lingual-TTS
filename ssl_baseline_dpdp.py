...
if __name__ == "__main__":
    ...
    # pr_model = SSLPRModel(
    #     system_type="pr-ssl-baseline-tune",
    #     ckpt_path="output/ckpt/tune/fscl/f594089276f24f4e842494377129b3f5/checkpoints/epoch=5-step=1500.ckpt"  # 4shot
    # )
    # pr_model = PostNetWrapper(pr_model, lang_id=6).eval().cuda()
    # dpdp = DPDPSSLUnit('hubert_large_ll60k', postnet=pr_model)
    # dpdp.cuda()
    # generate_ssl_units("pr-ssl-enzh-baseline-tune4", "./preprocessed_data/JSUT", dpdp, lambd=0)

    # pr_model = SSLPRModel(
    #     system_type="pr-ssl-baseline-tune",
    #     ckpt_path="output/ckpt/tune/fscl/da878d0902bf4776a5b0af4cc7ec9095/checkpoints/epoch=39-step=10000.ckpt"  # 4shot
    # )
    # pr_model = PostNetWrapper(pr_model, lang_id=6).eval().cuda()
    # dpdp = DPDPSSLUnit('hubert_large_ll60k', postnet=pr_model)
    # dpdp.cuda()
    # generate_ssl_units("pr-ssl-enzh-baseline-tune-oracle", "./preprocessed_data/JSUT", dpdp, lambd=0)

    pr_model = SSLPRModel(
        system_type="pr-ssl-baseline-tune",
        ckpt_path="output/ckpt/tune/fscl/459e4103123e4ba392016fa0d7bbc519/checkpoints/epoch=5-step=1500.ckpt"  # 4shot
    )
    pr_model = PostNetWrapper(pr_model, lang_id=8).eval().cuda()
    dpdp = DPDPSSLUnit('hubert_large_ll60k', postnet=pr_model)
    dpdp.cuda()
    generate_ssl_units("pr-ssl-enzh-baseline-tune4", "./preprocessed_data/kss", dpdp, lambd=0)

    pr_model = SSLPRModel(
        system_type="pr-ssl-baseline-tune",
        ckpt_path="output/ckpt/tune/fscl/4836196208304707b79b6a8e1390d850/checkpoints/epoch=39-step=10000.ckpt"  # 4shot
    )
    pr_model = PostNetWrapper(pr_model, lang_id=8).eval().cuda()
    dpdp = DPDPSSLUnit('hubert_large_ll60k', postnet=pr_model)
    dpdp.cuda()
    generate_ssl_units("pr-ssl-enzh-baseline-tune-oracle", "./preprocessed_data/kss", dpdp, lambd=0)
