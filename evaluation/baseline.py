...
class SSLPRModel(pl.LightningModule):
    ...
    def forward(self, wav_path: str, lang_id):  # Since we can not get the correct length, batch inference is not implemented. 
        ...
        """
        repr, _ = self.pr_model.upstream.extract_from_paths([wav_path], norm=self._norm) 
        x = self.pr_model.downstream(repr, lengths=[repr.shape[1]])
...
def evaluate(
    ...
    for query in tqdm(queries):
        logits = res[query["basename"]]
        segment, phoneme = decoder.decode(-scipy.special.log_softmax(logits, axis=1), fp=20)
...
def main_jp():
    ...
    for s in [4, 8, 16]:
        ...
        for i in range(20):
            print(f"Save logits for {s} shot, task {i}...")
    ...
    for s in [4, 8, 16]:
        ...
        for i in range(20):
            ...
            evaluate(data_parser, task_path, logits_path, output_path, lang_id=6)
...
def main_ko():
    ...
    for s in [4, 8, 16]:
        ...
        for i in range(20):
            ...
            evaluate(data_parser, task_path, logits_path, output_path, lang_id=8)
