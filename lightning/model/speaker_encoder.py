...
class SpeakerEncoderOld(pl.LightningModule):
...
class SpeakerEncoder(pl.LightningModule):
    ...
    def __init__(self, model_config, spk_config):
        super().__init__()
        self.emb_type = spk_config["emb_type"]

        if self.emb_type == "table":
            n_speaker = len(spk_config["speakers"])
            self.model = nn.Embedding(n_speaker, model_config["transformer"]["encoder_hidden"])
        elif self.emb_type == "shared":
            self.model = nn.Embedding(1, model_config["transformer"]["encoder_hidden"])
        elif self.emb_type == "encoder":
    ...
    def forward(self, args):
        if self.emb_type == "table":
            ...
            # input()
            partial_embeds = self.model(ref_mels)
            assert not torch_exist_nan(partial_embeds)
            assert ref_slices[-1].stop <= len(partial_embeds)  # Very CAREFUL assertion for not accessing out of index.

            speaker_embeds = [partial_embeds[ref_slice].mean(dim=0) for ref_slice in ref_slices]
            for spk_emb in speaker_embeds:
                assert not torch_exist_nan(spk_emb)
            speaker_emb = torch.stack([torch.nn.functional.normalize(spk_emb, dim=0) for spk_emb in speaker_embeds])
            assert not torch_exist_nan(speaker_emb)
...
class LanguageEncoder(pl.LightningModule):
    ...
    def __init__(self, model_config, lang_config):
        super().__init__()
        self.emb_type = lang_config["emb_type"]

        if self.emb_type == "table":
            n_lang = 100  # currently fixed, enable up to 100 languages
            self.model = nn.Embedding(n_lang, model_config["transformer"]["encoder_hidden"])
        else:
            raise NotImplementedError
