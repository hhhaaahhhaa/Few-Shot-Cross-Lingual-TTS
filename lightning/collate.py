...
class LanguageCollate:
        ...
        for idxs in idx_arr:
            ...
            if re_id:
                ...
                if Define.UPSTREAM == "mel":
                    ref_phn_repr = self.calc_phn_repr3(data, sup_ids)
                else:
                    ref_phn_repr = self.calc_phn_repr2(data, sup_ids)
    ...
    def calc_phn_repr(self, data, idxs):
        ...
        for text, representation in zip(texts, representations):
            for t, r in zip(text, representation.transpose(1, 0, 2)):
                table[int(t)].append(r)

        phn_repr = np.zeros((n_symbols, 25, self.d_representation), dtype=float)
        for i in range(n_symbols):
            if len(table[i]) == 0:
                phn_repr[i] = np.zeros((25, self.d_representation))
            else:
                phn_repr[i] = np.mean(np.stack(table[i], axis=0), axis=0)

        return torch.from_numpy(phn_repr).float()

    def calc_phn_repr2(self, data, idxs):
        info = {}
        info["lang_id"] = data[idxs[0]]["language"]
        info["texts"] = [data[idx]["text"] for idx in idxs]
        info["ssl-wav"] = [torch.from_numpy(data[idx]["ssl-wav"]).float() for idx in idxs]
        info["ssl-duration"] = [data[idx]["ssl-duration"] for idx in idxs]

        return info

    def calc_phn_repr3(self, data, idxs):
        info = {}
        info["lang_id"] = data[idxs[0]]["language"]
        info["texts"] = [data[idx]["text"] for idx in idxs]
        info["representation"] = [torch.from_numpy(data[idx]["representation"]).float() for idx in idxs]
        info["duration"] = [data[idx]["duration"] for idx in idxs]
