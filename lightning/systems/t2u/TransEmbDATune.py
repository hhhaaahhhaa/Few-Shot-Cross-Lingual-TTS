...
class TransEmbDATuneSystem(System):
    ...
    def common_da_step(self, logits, logit_lengths, batch, batch_idx, train=True):
        f_label = torch.zeros(len(logits)).to(self.device).float()
        unsup_label = (batch[5] == LANG_NAME2ID[self.target_lang_id]).float()
        f_logits = F.softmax(logits[:, :-1, :], dim=2)
    ...
    def common_step(self, batch, batch_idx, train=True):
        ...
        t2u_batch, da_batch = batch["t2u"], batch["da"]
        t2u_loss_dict, t2u_output, alignments = self.common_t2u_step(t2u_batch, batch_idx, train)
        da_loss_dict = self.common_da_step(t2u_output, t2u_batch[7], da_batch, batch_idx, train)

        loss_dict = t2u_loss_dict
        loss_dict.update(da_loss_dict)
        loss_dict["Total Loss"] = loss_dict["T2U Loss"] + loss_dict["DA Loss"]
